...
export function ScreenshotEditor() {
  ...
  async function exportAll() {
    ...
    for (const locale of locales) {
      ...
      for (const size of sizes) {
        ...
        for (let i = 0; i < currentSlides.length; i++) {
          ...
          try {
            const capture = await captureSlide(el, size.w, size.h, scale);
            const base64 = capture.dataUrl.split(",")[1] || "";
            const filename = `${String(i + 1).padStart(2, "0")}-${slide.layout}.${capture.extension}`;
    ...
    if (failed === 0) {
      toast.success(`Exported ${okCount} files (${summary})`);
    } else if (okCount === 0) {
  ...
  async function flattenPng(dataUrl: string) {
    const resp = await fetch("/api/flatten", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dataUrl }),
    });
    if (!resp.ok) {
      throw new Error(`Flatten failed: HTTP ${resp.status}`);
    }
    const json = (await resp.json()) as { ok: boolean; dataUrl?: string; error?: string };
    if (!json.ok || !json.dataUrl) {
      throw new Error(json.error || "Flatten failed");
    }
    return json.dataUrl;
  }
  ...
  async function captureSlide(
    ...
    try {
      ...
      try {
        return { dataUrl: await flattenPng(dataUrl), extension: "png" };
      } catch (e) {
        console.warn("PNG flatten failed; falling back to JPEG export", e);
        const jpegUrl = await toJpeg(el, {
          width: w,
          height: h,
          pixelRatio: 1,
          quality: 0.98,
        ...
        return { dataUrl: jpegUrl, extension: "jpg" };
      }
    } finally {
